...
$(document).ready(function() {
  ...
  $('.switchMora').click(function() {
    ...
    $.ajax({
          ...
          url:  "cambiarBloqueoCliente/"+id+"/" + block,
      })
  ...
  $(document).on('click','.botonDetalleCuentaVentas',function(){
    ...
    $.ajax({
       ...
       .done(function( data, textStatus, jqXHR ) {
            var newElement;
            if ( console && console.log ) {
              $('#titleVentasTitular').text('Ventas Asociadas: '+ data.nombreCliente + ' ' + data.apellidoPatCliente + ' ' + data.apellidoMatCliente);
           }
       })
       .fail(function( jqXHR, textStatus, errorThrown ) {
           if ( console && console.log ) {
  ...
  $(document).on('click','.botonDetalleDeuda',function(){
    ...
    $.ajax({
       ...
       .done(function( data, textStatus, jqXHR ) {
            var newElement;
            if ( console && console.log ) {
              $('#titleVentasTitular').text('Ventas Asociadas por Deuda');
              if($.isEmptyObject(data)){
                newElem='<div class="alert alert-success" id="nohayVentas">"No hay ventas para mostrar"</div>';
                $('#tBodyVentas').html(newElem);
              }
    ...
    $.ajax({
       ...
       .done(function( data, textStatus, jqXHR ) {
            if ( console && console.log ) {
              ...
              $('#labelDeudaText').text("Deuda Total:");
  ...
  $("#abonoCuentaModal").on('hidden.bs.modal', function () {
    ...
    //ABRE EL MODAL DE ABONOS DESDE LAS DEUDAS MENSUALES
